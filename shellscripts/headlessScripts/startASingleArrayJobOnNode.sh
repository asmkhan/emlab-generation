...
if [ -n "$PBS_ARRAYID" ]; then
java -d64 -server -Xms1024m -Xmx1024m -XX:-UseConcMarkSweepGC -XX:+UseG1GC -Drun.id=$JOBNAME-$PBS_ARRAYID -DSCENARIO_FOLDER=file://$NODESCENARIOFOLDER -Dresults.path=$TEMP/$dir -Dscenario.file=$SCENARIO -Djava.library.path=/home/asmkhan/ILOG/CPLEX_Studio1262/cplex/bin/x86-64_linux -jar $TEMP/$NEWJARNAME > $JOBNAME-$PBS_ARRAYID.elog
else
java -d64 -server -Xms1024m -Xmx1024m -XX:-UseConcMarkSweepGC -XX:+UseG1GC -Drun.id=$JOBNAME -DSCENARIO_FOLDER=file://$NODESCENARIOFOLDER -Dresults.path=$TEMP/$dir -Dscenario.file=$SCENARIO -Djava.library.path=/home/asmkhan/ILOG/CPLEX_Studio1262/cplex/bin/x86-64_linux -jar $TEMP/$NEWJARNAME > $JOBNAME.elog
fi
