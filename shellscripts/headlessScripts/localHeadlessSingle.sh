...
java -d64 -server -Xms1024m -Xmx1024m -XX:-UseConcMarkSweepGC -XX:+UseG1GC -Drun.id=$JOBNAME -DSCENARIO_FOLDER=$SCENARIOPATH -Dresults.path=$LOCALRESULTFOLDER/$JOBNAME -Dscenario.file=$SCENARIO".xml" -Djava.library.path=/opt/ibm/ILOG/CPLEX_Studio1262/cplex/bin/x86-64_linux $QUERYCOMMAND -jar $LOCALJARFILE
