...
public interface PpdpAnnualRepository extends GraphRepository<PpdpAnnual> {
    ...
    // @Query(value =
    // "g.v(producer).in('BIDDER').filter{it.__type__=='emlab.gen.domain.market.electricity.PpdpAnnual'}.propertyFilter('time',
    // FilterPipe.Filter.EQUAL, time).filter{it.status == 2}", type =
    // QueryType.Gremlin)
    // public Iterable<PpdpAnnual>
    // findAllAcceptedPpdpAnnualForGivenProducerAndTime(
