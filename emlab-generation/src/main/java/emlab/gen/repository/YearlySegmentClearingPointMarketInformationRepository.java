...
public interface YearlySegmentClearingPointMarketInformationRepository
    ...
    // @Query(value = "g.v(ppdp).out('BIDDINGMARKET').propertyFilter('time',
    // FilterPipe.Filter.EQUAL,
    // time).as('x').out('MARKET_INFORMATION_POINT').back('x')", type =
    // QueryType.Gremlin)
    // YearlySegmentClearingPointMarketInformation
    // findMarketInformationForPPDPAndTime(@Param("time") long time,
